<?php
	...
	function select4Latestmedicine($conn){
		$row = array();
		$query = "SELECT medicine_isbn, medicine_image FROM medicines ORDER BY medicine_isbn DESC";
	...
	function getmedicineByIsbn($conn, $isbn){
		$query = "SELECT medicine_title, medicine_author, medicine_price FROM medicines WHERE medicine_isbn = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getmedicineprice($isbn){
		$conn = db_connect();
		$query = "SELECT medicine_price FROM medicines WHERE medicine_isbn = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
	...
	function getPubName($conn, $pubid){
		$query = "SELECT publisher_name FROM publisher WHERE medicine_weight = '$pubid'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getAll($conn){
		$query = "SELECT * from medicines ORDER BY medicine_isbn DESC";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
